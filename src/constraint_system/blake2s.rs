...
impl StandardComposer {
    ...
    pub fn xor(&mut self, left: &mut [Variable], right: &[Variable]) {
        ...
        for i in 0..left.len() {
            ...
            let one_var = self.add_input(BlsScalar::one());
            left[i] = {
                //std::println!("left {:?} right {:?} out {:?} table {:?}", self.variables[&left[i]], self.variables[&right[i]], self.variables[&out_var], self.variables[&one_var]);
...
mod tests {
    ...
    fn test_blake2s_proof() {
        ...
        let mut prover = Prover::new(b"blake2s_proof");
        let n = 2usize.pow(14);
        ...
        // Commit Key
        let (ck, _) = public_parameters.trim(n).unwrap();
        ...
        let mut hash_input_vars = [prover.cs.zero_var; 32];
        for i in 0..32 {
            message_vars[i] = prover.cs.add_input(BlsScalar::from(message_bytes[i] as u64));
            hash_input_vars[i] = prover.cs.add_input(BlsScalar::from(hash_bytes[i] as u64));
        }

        let hash_output_vars = prover.cs.blake2s_256(message_vars);

        for i in 0..32 {
            prover.cs.assert_equal(hash_input_vars[i], hash_output_vars[i]);
        }

        std::println!("gates: {:?}", prover.cs.circuit_size());
